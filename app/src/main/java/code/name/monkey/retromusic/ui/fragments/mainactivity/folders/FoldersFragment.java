...
public class FoldersFragment extends AbsMainActivityFragment implements
  ...
  private void setUpAdapter() {
    adapter = new SongFileAdapter(getMainActivity(),
  ...
  public void onFileSelected(File file) {
    final File canonicalFile = FileUtil
        .safeGetCanonicalFile(file); // important as we compare the path value later
    if (canonicalFile.isDirectory()) {
      setCrumb(new BreadCrumbLayout.Crumb(canonicalFile), true);
    } else {
      FileFilter fileFilter = pathname -> !pathname.isDirectory() && audioFileFilter
          .accept(pathname);
      new ListSongsAsyncTask(getActivity(), null, (songs, extra) -> {
        ...
        if (startIndex > -1) {
          ...
          Snackbar.make(coordinatorLayout, Html.fromHtml(String
                  .format(getString(R.string.not_listed_in_media_store), canonicalFile.getName())),
              Snackbar.LENGTH_LONG)
              .setAction(R.string.action_scan,
                  v -> scanPaths(new String[]{canonicalFile.getPath()}))
              .setActionTextColor(ThemeStore.accentColor(getActivity()))
              .show();
        }
      }).execute(
          new ListSongsAsyncTask.LoadingInfo(toList(canonicalFile.getParentFile()), fileFilter,
              getFileComparator()));
    }
  }
  ...
  public void onMultipleItemAction(MenuItem item, ArrayList<File> files) {
    ...
    new ListSongsAsyncTask(getActivity(), null, (songs, extra) -> {
      ...
      if (songs.size() != files.size()) {
        Snackbar.make(coordinatorLayout, R.string.some_files_are_not_listed_in_the_media_store,
            Snackbar.LENGTH_LONG)
    ...
    }).execute(new ListSongsAsyncTask.LoadingInfo(files, audioFileFilter, getFileComparator()));
  }
      ...
      popupMenu.setOnMenuItemClickListener(item -> {
        ...
        switch (itemId) {
          ...
          case R.id.action_delete_from_device:
            ...
            }).execute(new ListSongsAsyncTask.LoadingInfo(toList(file), audioFileFilter,
                getFileComparator()));
            return true;
          ...
          case R.id.action_scan:
            new ListPathsAsyncTask(getActivity(), paths -> scanPaths(paths))
                .execute(new ListPathsAsyncTask.LoadingInfo(file, audioFileFilter));
            return true;
        }
        return false;
      });
            ...
            new ListSongsAsyncTask(getActivity(), null, (songs, extra) -> {
              if (!songs.isEmpty()) {
                ...
                Snackbar.make(coordinatorLayout, Html.fromHtml(
                    String.format(getString(R.string.not_listed_in_media_store), file.getName())),
                    Snackbar.LENGTH_LONG)
                    .setAction(R.string.action_scan,
                        v -> scanPaths(new String[]{FileUtil.safeGetCanonicalPath(file)}))
                    .setActionTextColor(ThemeStore.accentColor(getActivity()))
                    .show();
              }
            }).execute(new ListSongsAsyncTask.LoadingInfo(toList(file), audioFileFilter,
